...
public sealed class UrgentSignalConsumer(
    ...
    ILogger<UrgentSignalConsumer> _logger)
    : ICapSubscribe
{
    ...
    {
        _logger.LogDebug(
            "Processing batch of {Count} urgent signals for user {UserId}",
            batch.Signals.Count, batch.UserId);
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Processed batch of {Count} urgent signals for user {UserId}, generated {RecCount} recommendations via {Tier}",
                batch.Signals.Count, batch.UserId, outcome.GeneratedRecommendations.Count, outcome.FinalTier);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing urgent signal batch for user {UserId}", batch.UserId);
