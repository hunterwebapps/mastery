...
internal static class RagContextFormatter
{
    ...
    /// </summary>
    public static string? FormatForSelection(RagContext? context, DateOnly today)
    {
        if (context is null || context.Items.Count == 0)
            return null;
    ...
    /// </summary>
    public static void AppendForSelection(StringBuilder sb, RagContext? context, DateOnly today)
    {
        var formatted = FormatForSelection(context, today);
        if (formatted is not null)
        {
            sb.AppendLine(formatted);
        }
    ...
    private static string FormatRecency(RagContextItem item, DateOnly today)
    {
        ...
        if (!dateMatch.Success
            || !DateOnly.TryParse(dateMatch.Groups[1].Value,
                out var date))
        {
            return "recent";
        }
