...
public sealed class CosmosVectorStore : IVectorStore
{
    ...
    {
        this._logger = logger;
        this._container = cosmosClient.GetContainer(
    ...
    public async Task UpsertBatchAsync(IEnumerable<EmbeddingDocument> documents, CancellationToken ct)
    {
        ...
        {
            var batch = this._container.CreateTransactionalBatch(new PartitionKey(group.Key));
    ...
    public async Task DeleteAsync(string entityType, Guid entityId, CancellationToken ct)
    {
        ...
        var iterator = this._container.GetItemQueryIterator<dynamic>(query);
        ...
        while (iterator.HasMoreResults)
        {
            ...
            foreach (var item in response)
            {
                ...
                try
                {
                    await this._container.DeleteItemAsync<EmbeddingDocument>(
                        documentId,
        ...
        CancellationToken ct = default)
    {
        ...
        var iterator = this._container.GetItemQueryIterator<VectorSearchResultInternal>(
            queryDefinition,
