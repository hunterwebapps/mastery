...
#pragma warning restore OPENAI001
        ...
        // Extract usage and build call record
        ...
        var llmCallRecord = new LlmCallRecord(
            Stage: "Execution",
            Model: ExecutionPrompt.Model,
        ...
        logger.LogDebug(
            "OpenAI Execution response: {InputTokens} in / {OutputTokens} out / {CachedTokens} cached / {LatencyMs}ms",
            inputTokens,
            outputTokens,
            cachedInputTokens,
        ...
        if (failures.Count > 0)
        {
            ...
            return ExecutionResult.Failed($"Tool call(s) failed: {errors}", llmCallRecord);
        }
        ...
        {
            return ExecutionResult.Failed("No tool calls were executed", llmCallRecord);
        }
